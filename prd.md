...
| Day | Tasks |
| :--- | :--- |
| **Day 1** | Project setup - Express server, .env config, MongoDB connection, folder structure |
| **Day 2** | Groq API integration - groqService.js, test prompts, handle response parsing |
| **Day 3** | API routes + controller: POST /api/review full flow, save to DB, return response |
| **Day 4** | GET endpoints: fetch by ID, list all reviews; basic error handling |
| **Day 5** | Testing with Postman, clean up code, coordinate JSON contract with frontend team |

---

## 8. Notes & Assumptions
* [cite_start]Groq model: `llama3-8b-8192` (fast and free tier friendly). [cite: 69]
* [cite_start]No authentication required for this phase. [cite: 69]
* [cite_start]Frontend team will consume the JSON response directly. [cite: 69]
* [cite_start]All environment variables stored in `.env` never committed to Git. [cite: 69]
* [cite_start]Error responses follow a standard format: `{ error: 'message' }`. [cite: 69]
